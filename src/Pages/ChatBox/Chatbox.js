    ...
    <Layout userData={userData}>
      <div className="w-full p-3">
        ...
        <form onSubmit={sendMessage} className="pt-3 w-full inline-flex">
          <input
            className="border-2 rounded-3xl px-3 w-full py-1 outline-none focus:shadow"
            value={mssgVal}
            onChange={(e) => setMssgVal(e.target.value)}
            placeholder="Say something"
          />
          <button
            className={`material-icons border-2 p-2 px-4 mx-2 bg-white rounded-full transition-all duration-75 ease-in-out text-xl ${
              !mssgVal || "text-pink-700 hover:text-pink-900"
            }`}
            type="submit"
            disabled={!mssgVal}>
            send
